...
- **Cervical Fluid renders as separated tiles, not an overlay bar.** The original plan (Task 4) kept the single continuous `getCFBarHeight`-tall overlay; that painted over the 3px white gaps and read as one band, defeating the separated-tile design. Final implementation fills the **bottom-N sub-tiles directly** (N = `getCFBarHeight(appearance) / 28`) with `getCFBarColor(appearance)`, so the column reads as discrete stacked tiles. The colour/height map, the flow-precedence guard (`!cfData?.menstrualFlow`), logged-`NONE` rendering (fills the Dry tile), tail, and the spec hover rule (filled tiles keep the appearance colour; empty tiles deepen) are all preserved.
- **Unchanged from spec:** LH symbol stroke colours (Low/Rising/Declining `#3b82f6`; Peak `#16a34a` + amber `#f59e0b` dot), hover/crosshair/tooltip behaviour, the CF flow-precedence guard, menstrual-flow markers, and every layout invariant.
