...
- **CF fill is per sub-tile, not an overlay bar.** Task 4 kept the continuous `getCFBarHeight` overlay, which bridged the new 3px gaps and read as one band. Corrected post-review: the bottom-N sub-tiles (N = `getCFBarHeight/28`) are filled directly with `getCFBarColor(appearance)`; flow precedence, NONE→Dry-tile, tail, and the hover rule preserved.
- **Unchanged from plan/spec:** LH stroke colours (`#3b82f6` low/rising/declining; `#16a34a`+`#f59e0b` peak), CF flow-precedence guard, flow markers, hover/crosshair/tooltip, all layout invariants. Tests 291/291 green; eslint baseline unchanged.
